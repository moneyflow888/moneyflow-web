...
export default function InvestorsPage() {
  ...
  async function loadMyData() {
    ...
    // ✅ public share price
    try {
      const sp = await fetch("/api/public/share-price", { cache: "no-store" });
      const j = await sp.json();
      if (Number.isFinite(j?.share_price) && Number(j.share_price) > 0) {
        setSharePrice(Number(j.share_price));
  ...
  // 提款限制：最多可申請到「估值 - 已 pending」
  const pendingW = num(account?.pending_withdraw);
  const withdrawLimit = value !== null ? Math.max(0, value) : 0;
  const withdrawRemaining = Math.max(0, withdrawLimit - pendingW);
  ...
  async function submitWithdraw() {
    try {
      ...
      // ✅ 沒有 share_price 時，不讓提款（避免你遇到“沒錢也能申請”）
      if (sharePrice === null || value === null) {
        throw new Error("目前無法估值（share_price 尚未可用），暫時不能提交提款。");
      }

      // ✅ 不允許超過可申請額度
      if (amount > withdrawRemaining + 1e-9) {
        throw new Error(`超過可申請額度。可申請上限：${fmt(withdrawRemaining)} U`);
      }
        ...
        <div className="mt-10 space-y-4">
            ...
            <Card
              ...
              subtitle={
                <div className="text-sm" style={{ color: THEME.muted }}>
                  （你目前已提交但尚未結算）
                  {sharePrice !== null && value !== null ? (
                    <div className="mt-1" style={{ color: THEME.faint }}>
                      可提款上限：{fmt(withdrawLimit)} U；剩餘可再申請：{fmt(withdrawRemaining)} U
                    </div>
                  ) : null}
                </div>
              }
          ...
          >
            <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              <div className="rounded-2xl border p-4" style={{ borderColor: "rgba(255,255,255,0.10)" }}>
                ...
                {sharePrice !== null && value !== null ? (
                  <div className="mt-2 text-xs" style={{ color: THEME.faint }}>
                    可再申請額度：{fmt(withdrawRemaining)} U
                  </div>
                ) : (
                  <div className="mt-2 text-xs" style={{ color: THEME.faint }}>
                    （等待 share_price 可用後才能提交提款）
                  </div>
                )}
          ...
          <Card accent="gold" title="你的 profile" subtitle="來自 investor_profiles（RLS）。">
            ...
            <div className="mt-4 text-xs" style={{ color: THEME.faint }}>
              下一步：share_price 會跟著基金淨值波動，你的 PnL 會依照「你持有 shares」來變動。
            </div>
          </Card>
        </div>
      )}
