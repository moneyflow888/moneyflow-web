// app/api/admin/execute-withdrawals/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    for (const w of pending) {
      ...
      // 3.1 讀投資人 shares / pending_withdraw / principal
      const { data: acct, error: acctErr } = await supabase
        .from("investor_accounts")
        .select("user_id, shares, pending_withdraw, principal")
      ...
      // ✅ fund 層 principal_adjustments：Dashboard PnL Only 抵消出金（負數）
        ...
        .insert([
          {
            month: monthKeyNow(),
            delta: -amount, // 出金負數
            note: `auto: withdrawal executed user=${w.user_id} withdraw_id=${w.id}`,
            created_at: nowIso,
          },
        ]);
